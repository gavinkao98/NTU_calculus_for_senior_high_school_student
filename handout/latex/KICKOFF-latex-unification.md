...
| Phase | 內容 | DoD（驗收） | 狀態 |
|---|---|---|---|
| **P0 pilot** | ch03 源升格（`dist/ch03/chapter3.tex` → `src/ch03/`）＋編譯三閘＋`check_prose` 對現行 fragment 跑 PASS（證升格無損）；圖 harness 自 standalone 抽出→`export_figs` 重匯→與 `chapters/ch03/figs/` 現品比對一致 | 三閘綠＋子序列 PASS＋圖匯出一致 | ⏳ 2026-08-09 開跑 |
| **P1 源接管** | ch08＋appA/C/D 首轉（最後一批走 convert.py：方言盤點→mapping→四閘）；其餘 7 單元 tex 升格 `src/`；12 單元 figs harness 全建；`html/` 凍結歸檔、`_audit`/`_dev-archive` 活資產遷出；`make_dist.py` 改造為 src→pdf | 12 單元 src 編譯全綠；fragment 歸檔；convert.py／check_prose／build.py 退役歸檔 | ⬜ |
