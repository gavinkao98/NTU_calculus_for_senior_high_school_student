...
\begin{envdefinition}{Definition}{def:1.1}{}
...
\end{envdefinition}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.1}{}
...
\begin{workedexample}
\begin{envexample}{Example}{ex:1.2}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.3}{}
...
\begin{envdefinition}{Definition}{def:1.2}{}
...
\end{envdefinition}
\begin{envcaution}{Caution}{caut:1.1}{}
...
\end{envcaution}
\begin{envremark}{Remark}{rem:1.2}{}
...
\end{envremark}
\pagebreakbefore
\begin{envtheorem}{Theorem}{thm:1.1}{}
...
\end{envproof}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.4}{}
...
\begin{envproposition}{Proposition}{prop:1.1}{}
...
\end{envproposition}
Figure \ref{fig:1.2} illustrates both identities: \(f\) and \(f^{-1}\) travel between \(A\) and \(B\) in opposite directions, and composing them recovers the original input.

\begin{figureblock}
  \includegraphics[width=90.49mm]{ch01/figs/fig-map}
\figcaption{fig:1.2}{A one-to-one function and its inverse map between \(A\) and \(B\).}
\end{figureblock}
...
\begin{workedexample}
\begin{envexample}{Example}{ex:1.5}{}
...
\begin{envstrategy}{Strategy}{strat:1.1}{Finding the inverse of a one-to-one function}
\begin{steps}
  \item Write \(y = f(x)\).
...
\end{steps}
\end{envstrategy}
Note that this procedure assumes \(f\) is already one-to-one: applying it blindly to \(y = x^{2}\), for instance, yields \(x = \pm\sqrt{y}\): two values, not a function. Always verify one-to-one first, or restrict the domain (as in Example \ref{ex:1.5}).

\begin{workedexample}
\begin{envexample}{Example}{ex:1.6}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.7}{}
...
\end{envexample}
\begin{envsolution}{Solution}{}{}
Let \(y = \frac{3}{x - 5}\). Solving for \(x\): \[ x - 5 = \frac{3}{y}, \quad\text{so}\quad x = 5 + \frac{3}{y}. \] Interchanging the variables, \[ f^{-1}(x) = 5 + \frac{3}{x} = \frac{5x + 3}{x}, \qquad x \neq 0. \] Verification, in both directions: \[ \begin{aligned} f\!\bigl(f^{-1}(x)\bigr) &= \frac{3}{\bigl(5 + \tfrac{3}{x}\bigr) - 5} = \frac{3}{3/x} = x, \\ f^{-1}\!\bigl(f(x)\bigr) &= 5 + \frac{3}{3/(x - 5)} = 5 + (x - 5) = x. \end{aligned} \] Both compositions return the input, confirming Proposition \ref{prop:1.1}. Note how the domains pair up: \(f\) excludes \(x = 5\) and \(f^{-1}\) excludes \(x = 0\), matching the fact that \(f\) never takes the value \(0\) and \(f^{-1}\) never takes the value \(5\).
\end{envsolution}
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.8}{}
...
Consider the sine function on \(\mathbb{R}\). As Figure \ref{fig:1.4} shows, a horizontal line can intersect the graph of \(y = \sin x\) more than once, so \(\sin\) is not one-to-one on \(\mathbb{R}\).

\begin{figureblock}
  \includegraphics[width=132.82mm]{ch01/figs/sine-not-1to1}
\figcaption{fig:1.4}{The sine function on \(\mathbb{R}\) is not one-to-one.}
\end{figureblock}
\subsechead{The inverse sine function}
To define the inverse sine, we restrict \(\sin x\) to the interval \[ -\tfrac{\pi}{2} \le x \le \tfrac{\pi}{2}. \] On this interval, \(\sin x\) is one-to-one and its values cover the range \([-1, 1]\). The restricted branch is shown in Figure \ref{fig:1.5}.

\begin{figureblock}
  \includegraphics[width=90.49mm]{ch01/figs/restricted-sine}
\figcaption{fig:1.5}{Restricted branch of the sine function used to define \(\arcsin x\).}
\end{figureblock}
\begin{envdefinition}{Definition}{def:1.3}{}
...
\end{envdefinition}
\begin{envcaution}{Caution}{caut:1.2}{}
...
\end{envcaution}
\begin{envstrategy}{Strategy}{strat:1.2}{The reference-triangle method}
...
\end{envstrategy}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.9}{}
...
\figcaption{fig:1.6}{Right triangle with \(\sin\theta = \tfrac{1}{3}\).}
\end{figureblock}
...
The cosine function is one-to-one on the interval \(0 \le x \le \pi\), shown in Figure \ref{fig:1.7}.

\begin{figureblock}
  \includegraphics[width=90.49mm]{ch01/figs/restricted-cosine}
\figcaption{fig:1.7}{Restricted branch of the cosine function used to define \(\arccos x\).}
\end{figureblock}
\begin{envdefinition}{Definition}{def:1.4}{}
...
\end{envdefinition}
\begin{envproposition}{Proposition}{prop:1.3}{Inverse cosine identities}
\[ \begin{aligned} \cos(\arccos x) = x, &\qquad -1 \le x \le 1,\\ \arccos(\cos x) = x, &\qquad 0 \le x \le \pi. \end{aligned} \]
\end{envproposition}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.10}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.11}{}
...
On the open interval \(\left(-\tfrac{\pi}{2}, \tfrac{\pi}{2}\right)\), the tangent function climbs continuously from \(-\infty\) to \(+\infty\), taking each real number as a value exactly once. This is why the inverse tangent, unlike \(\arcsin\) and \(\arccos\), accepts \emph{every} real number as input: its domain is all of \(\mathbb{R}\) (Figure \ref{fig:1.9}).

The tangent function is one-to-one on the interval \(-\tfrac{\pi}{2} < x < \tfrac{\pi}{2}\), shown in Figure \ref{fig:1.8}.

\begin{figureblock}
  \includegraphics[width=79.9mm]{ch01/figs/restricted-tangent}
\figcaption{fig:1.8}{Restricted branch of the tangent function used to define \(\arctan x\).}
\end{figureblock}
\begin{envdefinition}{Definition}{def:1.5}{}
...
\end{envdefinition}
Figure \ref{fig:1.9} shows this reflection. In the reflected graph, the vertical asymptotes of the restricted tangent branch become horizontal asymptotes of the inverse tangent graph.

\begin{figureblock}
  \includegraphics[width=79.9mm]{ch01/figs/arctan-reflection}
\figcaption{fig:1.9}{The restricted tangent branch and its inverse, reflected across \(y = x\).}
\end{figureblock}
\begin{envproposition}{Proposition}{prop:1.4}{Inverse tangent identities}
\[ \begin{aligned} \tan(\arctan x) = x, &\qquad x \in \mathbb{R},\\ \arctan(\tan x) = x, &\qquad -\tfrac{\pi}{2} < x < \tfrac{\pi}{2}. \end{aligned} \]
\end{envproposition}
\begin{envcaution}{Caution}{caut:1.4}{}
...
Dividing the Pythagorean identity \(\sin^{2}\theta + \cos^{2}\theta = 1\) through by \(\cos^{2}\theta\) gives the companion identity \(1 + \tan^{2}\theta = \sec^{2}\theta\), which Example \ref{ex:1.13} uses. Note the contrast with Caution \ref{caut:1.2}: the superscript in \(\sin^{-1} x\) does \emph{not} denote a reciprocal, but \(\sec\) and \(\csc\) really are reciprocals (and \(\cot\) is the inverted ratio \(\cos/\sin\)). Appendix A.1 develops these three functions more fully.

\begin{workedexample}
\begin{envexample}{Example}{ex:1.12}{}
...
\figcaption{fig:1.10}{Right triangle with \(\tan\theta = 10\).}
\end{figureblock}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.13}{}
...
\figcaption{fig:1.11}{Right triangle with \(\tan y = x\).}
\end{figureblock}
\end{envsolution}
\end{workedexample}
...
\begin{envdefinition}{Definition}{def:1.6}{}
...
\end{envdefinition}
\begin{envdefinition}{Definition}{def:1.7}{}
...
\end{envdefinition}
\begin{envdefinition}{Definition}{def:1.8}{}
...
\begin{envremark}{Remark}{rem:1.3}{}
...
\end{envremark}
\begin{envremark}{Remark}{rem:1.4}{}
...
\end{envremark}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.14}{}
...
Under the alternative convention of Remark \ref{rem:1.4} (\(\arccsc(-1) = -\tfrac{\pi}{2}\)) one gets \(f(-1) = -\pi\) instead. This is a concrete instance of branch conventions changing answers, and it is why this text fixes its conventions explicitly.
\end{envsolution}
\end{workedexample}
...
\begin{envdefinition}{Definition}{def:1.9}{}
...
Notice that the definition deliberately says nothing about \(f(a)\): when we consider \(\lim_{x \to a} f(x)\), we are concerned with the values of \(f(x)\) for \(x\) near \(a\), not with the value of \(f(a)\). In particular, \(f(a)\) need not be defined at all. Figure \ref{fig:1.12} shows three functions whose values at \(a\) are different (and one is undefined), yet which all share the same limit as \(x \to a\).
...
\figcaption{fig:1.12}{Three functions with the same limit as \(x \to a\).}
\end{figureblock}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.15}{}
The graph of a function \(f\) is shown in Figure \ref{fig:1.13}. Evaluate the following, or state that the limit does not exist:
...
\figcaption{fig:1.13}{A function whose limit at \(x = 2\) differs from its value there.}
\end{figureblock}
\end{envexample}
\begin{envsolution}{Solution}{}{}
\begin{enumerate}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.16}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.17}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.18}{}
...
\end{envsolution}
\end{workedexample}
\begin{envcaution}{Caution}{caut:1.5}{}
\begin{raggedpara}
...
\begin{envdefinition}{Definition}{def:1.10}{}
...
\figcaption{fig:1.14}{A function whose left-hand and right-hand limits at \(a\) are different.}
\end{figureblock}
The two one-sided limits together determine the two-sided limit.

\begin{envproposition}{Proposition}{prop:1.5}{Criterion for a two-sided limit}
...
\begin{workedexample}
\begin{envexample}{Example}{ex:1.19}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.20}{}
...
\figcaption{fig:1.15}{The jump at \(x = 2\) in Example \ref{ex:1.20}.}
\end{figureblock}
\end{envsolution}
\end{workedexample}
...
\begin{workedexample}
\begin{envexample}{Example}{ex:1.21}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.22}{}
...
\figcaption{fig:1.16}{Comparing \(1/x^{2}\) and \(1/x\) near \(0\).}
\end{figureblock}
\end{envsolution}
\end{workedexample}
...
\begin{envdefinition}{Definition}{def:1.11}{}
...
\end{envdefinition}
\begin{envcaution}{Caution}{caut:1.6}{}
...
\end{envcaution}
One-sided versions, \[ \begin{aligned} \lim_{x \to a^{-}} f(x) &= \infty, &\qquad \lim_{x \to a^{+}} f(x) &= \infty, \\ \lim_{x \to a^{-}} f(x) &= -\infty, &\qquad \lim_{x \to a^{+}} f(x) &= -\infty, \end{aligned} \] are defined analogously. The four cases are illustrated in Figure \ref{fig:1.17}.
...
\begin{figureblock}
  ...
  \figcaption{fig:1.17}{The four typical one-sided infinite limits.}
  \end{minipage}
\end{figureblock}
...
\begin{envdefinition}{Definition}{def:1.12}{}
...
\end{envdefinition}
\begin{envstrategy}{Strategy}{strat:1.3}{Locating vertical asymptotes}
\begin{steps}
...
\begin{envtheorem}{Theorem}{thm:1.2}{Limit laws}
...
\begin{envcaution}{Caution}{caut:1.7}{}
...
\begin{workedexample}
\begin{envexample}{Example}{ex:1.26}{}
...
\begin{envproposition}{Proposition}{prop:1.6}{}
...
\begin{workedexample}
\begin{envexample}{Example}{ex:1.27}{}
...
\begin{workedexample}
\begin{envexample}{Example}{ex:1.28}{}
\begin{enumerate}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.29}{}
...
\figcaption{fig:1.20}{The removable hole at \(x = 1\); its height is \(\tfrac{1}{2}\).}
\end{figureblock}
\end{envsolution}
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.30}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.31}{}
...
\begin{workedexample}
\begin{envexample}{Example}{ex:1.32}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.33}{}
...
\figcaption{fig:1.21}{The greatest integer function \([x]\) has a jump at every integer.}
\end{figureblock}
\end{envsolution}
\end{workedexample}
...
\begin{envtheorem}{Theorem}{thm:1.3}{Squeeze theorem}
...
\figcaption{fig:1.22}{The squeeze theorem: \(f\) is trapped between \(g\) and \(h\), which share the same limit \(L\).}
\end{figureblock}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.34}{}
...
\figcaption{fig:1.23}{The graph of \(y = x^{2}\sin(1/x)\) squeezed between \(y = -x^{2}\) and \(y = x^{2}\).}
\end{figureblock}
\end{envsolution}
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.35}{}
...
\end{envsolution}
\end{workedexample}
\begin{envremark}{Remark}{rem:1.5}{}
...
\end{envremark}
\begin{envstrategy}{Strategy}{strat:1.4}{Computing a limit \(\lim_{x \to a} f(x)\)}
\begin{steps}
...
\end{envstrategy}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.36}{}
...
\begin{envremark}{Remark}{rem:1.6}{Historical note}
...
\begin{workedexample}
\begin{envexample}{Example}{ex:1.37}{}
...
\begin{envdefinition}{Definition}{def:1.13}{}
...
\end{informal}
\end{envdefinition}
Figure \ref{fig:1.24} gives a geometric reading of the definition. If \(x\) is restricted to the punctured interval \((a - \delta, a + \delta)\) with \(x \neq a\), then the corresponding values of \(f(x)\) lie in the target strip \((L - \varepsilon, L + \varepsilon)\). The value \(f(a)\) itself may be different from \(L\): the limit depends only on the behavior of \(f(x)\) when \(x\) is close to \(a\), not on what happens at \(x = a\).

\begin{figureblock}
  \includegraphics[width=95.78mm]{ch01/figs/precise-limit}
\figcaption{fig:1.24}{A punctured \(\delta\)-neighborhood maps into an \(\varepsilon\)-strip around \(L\).}
\end{figureblock}
The condition \(0 < |x - a| < \delta\) says that \(x\) lies within distance \(\delta\) of \(a\), but \(x \neq a\). The inequality \(|f(x) - L| < \varepsilon\) says that \(f(x)\) lies within distance \(\varepsilon\) of \(L\). The key phrase is “for every \(\varepsilon > 0\)”: no matter how small a tolerance \(\varepsilon\) we demand, we must be able to find a corresponding \(\delta\) (Figure \ref{fig:1.25}).
...
\figcaption{fig:1.25}{For \(f(x) = 2x - 1\) at \(a = 3\), tightening \(\varepsilon\) forces a narrower \(\delta\)-band.}
\end{figureblock}
...
\begin{workedexample}
\begin{envexample}{Example}{ex:1.38}{}
...
\begin{envproposition}{Proposition}{prop:1.7}{Uniqueness of limits}
...
\begin{workedexample}
\begin{envexample}{Example}{ex:1.39}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.40}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:1.41}{}
...
\begin{envstrategy}{Strategy}{strat:1.5}{Verifying a limit from the \(\varepsilon\)-\(\delta\) definition}
...
\end{steps}
\end{envstrategy}
The \(\varepsilon\)-\(\delta\) machinery also gives precise meaning to the informal definitions of infinite limits from Definition \ref{def:1.11}.

\begin{envdefinition}{Definition}{def:1.14}{}
...
\begin{workedexample}
\begin{envexample}{Example}{ex:1.43}{}
Show from the precise definition that \[ \lim_{x \to 0} \frac{1}{x^{2}} = \infty. \]
\end{envexample}
\begin{envsolution}{Solution}{}{}
Given \(M > 0\), choose \[ \delta = \frac{1}{\sqrt{M}}. \] If \(0 < |x| < \delta\), then \(x^{2} < \delta^{2} = 1/M\), so \[ \frac{1}{x^{2}} > M. \] Since every threshold \(M\) is eventually exceeded once \(x\) is close enough to \(0\), we have \(\lim_{x \to 0} \frac{1}{x^{2}} = \infty\). This is the rigorous counterpart of the informal argument in Example \ref{ex:1.21}.
\end{envsolution}
\end{workedexample}
...
Chapter 1 laid down two foundations that look very different from each other. The first was the inverse of a function (§1.1–§1.2). A function can be run backwards exactly when it is \emph{one-to-one}: distinct inputs give distinct outputs. The horizontal line test reads this condition straight off a graph. For such a function the inverse \(f^{-1}\) exists (Theorem \ref{thm:1.1}), described exactly by the identities \(f^{-1}(f(x)) = x\) and \(f(f^{-1}(y)) = y\) (Proposition \ref{prop:1.1}). Restricting each trigonometric function to a principal interval on which it is one-to-one produced the inverse trigonometric functions \(\arcsin\), \(\arccos\), \(\arctan\) and their three companions, with fixed principal ranges and the reference-triangle method (Strategy \ref{strat:1.2}) turning expressions such as \(\cos(\arcsin x)\) into algebra.

The second, and longer, thread was the \emph{limit} (§1.3–§1.6). We began with the informal idea that \(\lim_{x \to a} f(x) = L\) means that \(f(x)\) is as close to \(L\) as we please once \(x\) is close enough to, but not equal to, \(a\). We estimated such limits from tables and graphs (§1.3). One-sided limits refined the picture, with a two-sided limit existing exactly when the two one-sided limits agree (Proposition \ref{prop:1.5}); allowing the values to grow without bound gave infinite limits and the vertical asymptotes that mark them (§1.4). The limit laws (Theorem \ref{thm:1.2}) then made limits computable: for functions built from polynomials and rational functions, direct substitution works wherever the function is defined (Proposition \ref{prop:1.6}), and the indeterminate form \(0/0\) is handled by factoring, rationalization, or the squeeze theorem (Theorem \ref{thm:1.3}) (§1.5).

The precise \(\varepsilon\)-\(\delta\) definition (Definition \ref{def:1.13}) finally placed every preceding limit argument on a rigorous footing (§1.6): \(\lim_{x \to a} f(x) = L\) means that for every \(\varepsilon > 0\) there is a \(\delta > 0\) with \(\lvert f(x) - L \rvert < \varepsilon\) whenever \(0 < \lvert x - a \rvert < \delta\), and from it we proved that a limit, when it exists, is unique (Proposition \ref{prop:1.7}). The chapter closed by naming continuity, the property that limits make precise. A function \(f\) is continuous at \(a\) when \(\lim_{x \to a} f(x) = f(a)\). Chapter 2 is built on this machinery: the derivative is itself a limit, the limit of secant slopes.
\end{document}
