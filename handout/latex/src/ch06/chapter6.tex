...
\begin{workedexample}
\begin{envexample}{Example}{ex:6.1}{}
...
\begin{figureblock}
  ...
  \figcaption{fig:6.2}{Refining the right endpoint sum for \(y = x^{2}\) on \([0, 1]\). With \(n = 4, 8, 16\) strips the rectangles overshoot the curve on every strip, but by less and less as \(n\) grows, so the overestimates close in on the true area from above.}
  \end{minipage}
\end{figureblock}
\begin{workedexample}
\begin{envexample}{Example}{ex:6.2}{}
...
The exact area is \(\tfrac{1}{3}\), comfortably inside the bracket \(0.219 < A < 0.469\) from Example \ref{ex:6.1}. The same limit results from the left endpoint sums \(L_{n}\): they differ from \(R_{n}\) by only the single end rectangle \(\tfrac{1}{n}\bigl(f(1) - f(0)\bigr) = \tfrac{1}{n}\), which itself tends to \(0\). Approaching the area from below or from above, we arrive at the one number. \qedmark
\end{envsolution}
\end{workedexample}
That the left and right sums converge to a single value is exactly what lets us \emph{define} the area to be that value.

\begin{envdefinition}{Definition}{def:6.1}{Area under a curve}
...
\begin{workedexample}
\begin{envexample}{Example}{ex:6.3}{}
...
\begin{envdefinition}{Definition}{def:6.2}{Definite integral}
...
the \emph{net signed area} between the graph and the axis. For a non-negative \(f\) nothing lies below the axis, and this reduces to the area of Definition \ref{def:6.1}. The definite integral extends that idea to functions that change sign.

\begin{workedexample}
\begin{envexample}{Example}{ex:6.4}{}
Evaluate \(\displaystyle\int_{0}^{1} x^{2}\,dx\) directly from the definition.
\end{envexample}
\begin{envsolution}{Solution}{}{}
The integrand \(x^{2}\) is continuous, so Theorem \ref{thm:6.1} guarantees that the limit exists and that it does not depend on the choice of sample points. We may therefore use the right-endpoint sums already computed in §6.1 (Example \ref{ex:6.2}), where they were found to converge to \(\tfrac{1}{3}\). Hence
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:6.5}{}
...
\figcaption{fig:6.4}{The integral \(\int_{0}^{2} (x - 1)\,dx\) as net signed area. The line crosses the axis at \(x = 1\); the triangle below the axis (marked \(-\), area \(\tfrac{1}{2}\)) cancels the congruent triangle above it (marked \(+\), area \(\tfrac{1}{2}\)), so the integral is \(0\) even though the total area enclosed is \(1\).}
\end{figureblock}
\begin{workedexample}
\begin{envexample}{Example}{ex:6.6}{}
...
\figcaption{fig:6.5}{Reading \(\int_{-1}^{1} \sqrt{1 - x^{2}}\,dx\) as an area (Example \ref{ex:6.6}). The graph \(y = \sqrt{1 - x^{2}}\) is the upper half of the unit circle, so the shaded region is a semicircle of radius \(1\); its area \(\tfrac{\pi}{2}\) is the value of the integral.}
\end{figureblock}
...
\begin{envtheorem}{Theorem}{thm:6.2}{Properties of the definite integral}
...
\begin{workedexample}
\begin{envexample}{Example}{ex:6.7}{}
Given that \(\displaystyle\int_{0}^{1} x^{2}\,dx = \tfrac{1}{3}\) (Example \ref{ex:6.4}), and without evaluating any new limit, find \(\displaystyle\int_{0}^{1} \bigl(3x^{2} - 4\bigr)\,dx\).
\end{envexample}
\begin{envsolution}{Solution}{}{}
The properties turn one known integral into many. Linearity (Theorem \ref{thm:6.2}, part 2) splits the integral and pulls out the constant factor, and the constant rule (part 1) evaluates the remaining piece directly:
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:6.8}{}
Show that \(\displaystyle 3 \le \int_{1}^{4} \sqrt{x}\,dx \le 6\) without evaluating the integral.
\end{envexample}
\begin{envsolution}{Solution}{}{}
Since \(\sqrt{x}\) is continuous on \([1, 4]\), it is integrable there by Theorem \ref{thm:6.1}, so the properties apply. On \([1, 4]\) the integrand is increasing, running from \(\sqrt{1} = 1\) up to \(\sqrt{4} = 2\), so \(1 \le \sqrt{x} \le 2\) throughout, and the bounding property (Theorem \ref{thm:6.2}, part 4) with \(m = 1\), \(M = 2\), and \(b - a = 3\) gives
...
\begin{envdefinition}{Definition}{def:6.3}{Antiderivative}
...
\figcaption{fig:6.7}{The accumulation function \(g(x) = \int_{a}^{x} f(t)\,dt\) is the shaded area up to \(x\). Advancing \(x\) by a small \(h\) adds the thin strip over \([x, x + h]\), of width \(h\) and height about \(f(x)\), so \(g\) grows by roughly \(f(x)\,h\). Its rate of change is the current height \(f(x)\). The strip is widened for visibility.}
\end{figureblock}
\begin{envtheorem}{Theorem}{thm:6.3}{Fundamental Theorem of Calculus, Part 1}
...
Fix \(x\) in \((a, b)\) and take \(h \ne 0\) small enough that \(x + h\) lies in \([a, b]\). By additivity of the integral (Theorem \ref{thm:6.2}),
...
Consider the difference quotient \(\dfrac{g(x + h) - g(x)}{h} = \dfrac{1}{h}\displaystyle\int_{x}^{x+h} f(t)\,dt\). We claim it lies between the least and greatest values of \(f\) on the closed interval joining \(x\) and \(x + h\). For \(h > 0\), the comparison bound (Theorem \ref{thm:6.2}, part 4) applied on \([x, x+h]\) gives, with \(m_{h}\) and \(M_{h}\) the minimum and maximum of \(f\) there,
...
\end{envcaution}
\begin{workedexample}
\begin{envexample}{Example}{ex:6.9}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:6.10}{}
...
\begin{envtheorem}{Theorem}{thm:6.4}{Fundamental Theorem of Calculus, Part 2}
...
\begin{envstrategy}{Strategy}{strat:6.1}{Evaluating a definite integral}
...
\end{envstrategy}
\begin{workedexample}
\begin{envexample}{Example}{ex:6.11}{}
...
In Example \ref{ex:6.8} the bounding property could only confine this integral to the range \([3, 6]\); the Fundamental Theorem gives the exact value \(\tfrac{14}{3} \approx 4.67\), which indeed sits there. \qedmark
\end{envsolution}
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:6.12}{}
...
\begin{envdefinition}{Definition}{def:6.4}{Indefinite integral}
...
\begin{workedexample}
\begin{envexample}{Example}{ex:6.13}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:6.14}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:6.15}{}
...
\begin{envtheorem}{Theorem}{thm:6.5}{Net Change Theorem}
...
\end{envtheorem}
This is nothing more than Part 2 of the Fundamental Theorem (Theorem \ref{thm:6.4}) with the integrand written as \(F'\): \(F\) is an antiderivative of \(F'\) on \([a,b]\). Strictly, Theorem \ref{thm:6.4} asks for an antiderivative on an open interval around \([a,b]\), and the one-step construction that closes this gap is carried out in the proof of Theorem 8.2 (§8.1). What is new is how we read it. If \(V'(t)\) is the rate at which water flows into a tank, \(\int_{t_{1}}^{t_{2}} V'(t)\,dt\) is the net volume added; if \(C'(x)\) is a marginal cost, \(\int_{x_{1}}^{x_{2}} C'(x)\,dx\) is the added cost of raising production from \(x_{1}\) to \(x_{2}\). The most important case is motion.
...
\begin{workedexample}
\begin{envexample}{Example}{ex:6.16}{}
...
\begin{envtheorem}{Theorem}{thm:6.6}{The Substitution Rule}
...
\begin{envstrategy}{Strategy}{strat:6.2}{Integration by substitution}
...
\end{envstrategy}
\begin{workedexample}
\begin{envexample}{Example}{ex:6.17}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:6.18}{}
...
\end{workedexample}
\begin{workedexample}
\begin{envexample}{Example}{ex:6.19}{}
...
\begin{envtheorem}{Theorem}{thm:6.7}{Substitution in a definite integral}
If \(g'\) is continuous on \([a, b]\) and \(f\) is continuous on the range of \(g\), then

\[ \int_{a}^{b} f\bigl(g(x)\bigr)\,g'(x)\,dx = \int_{g(a)}^{g(b)} f(u)\,du. \]
\end{envtheorem}
\begin{envproof}{Proof}{}{}
Let \(F\) be an antiderivative of \(f\). As in Theorem \ref{thm:6.6}, \(F(g(x))\) is an antiderivative of \(f(g(x))\,g'(x)\), so by Part 2 of the Fundamental Theorem (Theorem \ref{thm:6.4}),
...
\end{envcaution}
\begin{workedexample}
\begin{envexample}{Example}{ex:6.20}{}
...
\begin{envtheorem}{Theorem}{thm:6.8}{Integrals of symmetric functions}
...
\end{envproof}
\begin{workedexample}
\begin{envexample}{Example}{ex:6.21}{}
Evaluate \(\displaystyle\int_{-1}^{1} \bigl(x^{4} + x^{3}\bigr)\,dx\).
\end{envexample}
\begin{envsolution}{Solution}{}{}
The integrand splits into an even part and an odd part. Over the symmetric interval \([-1, 1]\), the even \(x^{4}\) doubles its integral over \([0, 1]\), while the odd \(x^{3}\) integrates to zero (Theorem \ref{thm:6.8}):
...
This chapter built the integral from the ground up and bound it to the derivative. Section 6.1 met the area and distance problems and discovered them to be one limit of Riemann sums. Section 6.2 named that limit the \emph{definite integral} \(\int_{a}^{b} f(x)\,dx\) (Definition \ref{def:6.2}), established that a continuous integrand is integrable (Theorem \ref{thm:6.1}, taken on credit), read the integral as net signed area, and collected its properties (Theorem \ref{thm:6.2}).

Section 6.3 proved the \emph{Fundamental Theorem of Calculus}: Part 1 (Theorem \ref{thm:6.3}) shows the accumulation function \(\int_{a}^{x} f\) is an antiderivative of \(f\), so differentiating an integral returns the integrand; Part 2 (Theorem \ref{thm:6.4}) evaluates any definite integral from a single antiderivative. That the derivative of an accumulated area is the height, and that an area is therefore recovered from an antiderivative, is the central fact of the subject.

Section 6.4 introduced the notation \(\int f(x)\,dx\) for the \emph{indefinite integral} (Definition \ref{def:6.4}) and a working table, singled out \(\int \tfrac{1}{x}\,dx = \ln\lvert x\rvert + C\), and reread the theorem as the Net Change Theorem (Theorem \ref{thm:6.5}). Section 6.5 turned the chain rule around into the Substitution Rule (Theorems \ref{thm:6.6}–\ref{thm:6.7}) and used it to exploit symmetry (Theorem \ref{thm:6.8}).
