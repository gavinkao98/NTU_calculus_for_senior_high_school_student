...
\begin{caution}
A common informal description of the tangent line --- ``the line that touches the curve at exactly one point'' --- is not the right idea. A line can touch a curve at one point without being tangent to it (a vertical line touches \(y = x^{3}\) at one point but is not its tangent), and a tangent line can touch the curve at more than one point (the tangent to \(y = \sin x\) at \(x = 0\) is \(y = x\), which meets the curve again at every integer multiple of \(\pi\) where \(\sin x\) crosses \(y = x\)). The mathematically correct picture is the limiting-secant one of \cref{def:tangent-line}: we ask for the line whose slope is the limit of nearby secant slopes, not for the line that ``touches at one point''.
\end{caution}
...
\end{enumerate}
The result is a polynomial whose degree is one less than the original. The first non-zero term of the original polynomial (the constant) becomes \(0\) after differentiation, which is why the degree drops by one.
\end{strategy}
...
\[
\frac{d}{dx}\bigl[b^{x}\bigr] = b^{x} \cdot \log_{e} b,
\]
where the proportionality factor \(\log_{e} b\) (the natural logarithm of \(b\)) reduces to \(1\) exactly when \(b = e\). The derivation routes through the chain rule applied to \(b^{x} = e^{x \log b}\); we will give it once the chain rule is available.
\end{remark}
