...
\begin{proposition}\label{prop:cauchy-bounded}
...
\begin{proposition}[Monotone subsequence]\label{prop:monotone-subsequence}
...
\begin{proof}
By \cref{prop:monotone-subsequence}, the sequence has a monotone subsequence; the subsequence is bounded (any subsequence of a bounded sequence is bounded). A bounded monotone sequence converges by \cref{thm:completeness}.
\end{proof}

\begin{proof}[Proof of \cref{thm:cauchy-iff-convergent}, \(\Leftarrow\) direction]
Let \(\{a_{n}\}\) be Cauchy. By \cref{prop:cauchy-bounded} it is bounded; by \cref{thm:bolzano-weierstrass} it has a convergent subsequence \(\{a_{n_{k}}\}\) with limit \(L\) for some \(L \in \mathbb{R}\). We claim the full sequence converges to \(L\).
...
\begin{proposition}\label{prop:eh-minus-1-over-h-bound}
...
\begin{proof}
By the exponent law (\cref{thm:exponent-law}) and \cref{prop:eh-minus-1-over-h-bound},
\[
